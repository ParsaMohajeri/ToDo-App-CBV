...
class TaskEditView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        ...
        return Task.objects.filter(
            author=self.request.user, deadline__gt=timezone.now()
        ) | Task.objects.filter(author=self.request.user, deadline__isnull=True)

    def get(self, request, *args, **kwargs):
        task = self.get_object()
        if task.deadline and task.deadline <= timezone.now():
            return HttpResponseForbidden(
                "This task's deadline has passed and cannot be edited."
            )
