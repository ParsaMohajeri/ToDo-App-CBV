...
@pytest.fixture
def user():
    return User.objects.create_user(username="parsa",email="test234445@gmail.com", password="test/@pass")

@pytest.fixture
def other_user():
    return User.objects.create_user(username="other",email="test23444ve5@gmail.com", password="other/@pass")
...
class TestTaskViewSet:
    def test_list_authenticated_user_sees_own_tasks(self, api_client, user, task, other_user):
        Task.objects.create(
            author=other_user,
            title="Other Task",
            content="Hidden",
